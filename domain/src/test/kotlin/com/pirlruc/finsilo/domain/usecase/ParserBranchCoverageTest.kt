...
class ParserBranchCoverageTest {
    ...
    fun trading212TradeRowSkipsBlankTickerAndZeroQty() {
        ...
        assertTrue(parsed.lines.any { it.type == TransactionType.INTEREST })
    ...
    fun brokerDetectHitsEachHeaderAlias() {
        ...
        assertEquals(BrokerCsvFormat.DEGIRO_TRANSACTIONS, BrokerCsvDetect.from(listOf("Aantal", "Quantity", "Koers", "Price", "Product")))
        assertEquals(BrokerCsvFormat.DEGIRO_TRANSACTIONS, BrokerCsvDetect.from(listOf("Produto", "Quantidade", "Preço")))
        assertEquals(BrokerCsvFormat.DEGIRO_ACCOUNT, BrokerCsvDetect.from(listOf("Descrição", "Alteração", "Saldo")))
        assertEquals(BrokerCsvFormat.REVOLUT_STOCKS, BrokerCsvDetect.from(listOf("Ticker", "Tipo", "Precio por acción")))
        assertEquals(BrokerCsvFormat.REVOLUT_STOCKS, BrokerCsvDetect.from(listOf("Ticker", "Type", "Price")))
        assertNull(BrokerCsvDetect.from(listOf("Koers", "Product")))
