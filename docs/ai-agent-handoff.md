...
- Hash/wrap dual-write uses compensating rollback (`rollbackLastWrap`) for setup, recovery rotation, wrap upgrade, and recovery PIN reset. Unifying `finsilo_lock` and `finsilo_secure` into one commit is still [FS-033-T2](issues.yml).
...
*Last updated: 2026-09-19 (lock/HTTP/backup security fixes; FS-034 TLS pinning epic)*
