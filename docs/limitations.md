...
| LIM-MI-UI | multimetric SEI MI is enforced on **`:domain` production Kotlin** (KT-CPLX-002, gate `min_maintainability_index`). Compose screens (`DashboardScreen`, `LedgerEntryScreen`, `LedgerEntryViewModel`) score below 40 because per-file MI is dominated by `@Composable` length. | Split those screens into smaller composables/files, then include `:app` in `scripts/check-maintainability.py`. | **Open — Compose file MI** | `:app` UI sources are in the MI job and min ≥ 40 |
